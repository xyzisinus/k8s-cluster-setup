...
# a worker node, this script can be executed on the worker.
# k8s-setup.sh follows the steps above to configure the k8s cluster
# one node at a time.
#
...
# k8s-local-setup.sh may also be able to figure out the nodes in the
# cluster and whether it's running on the master, thus this script
# can be executed without the nodes as arguments under such conditions.
